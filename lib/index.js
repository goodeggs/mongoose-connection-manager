    ...
    connect: function(cb) {
      ...
      connectTo = (function(_this) {
        return function(name, settings) {
          var finishOrRetry, options, url, _ref;
          url = settings.url;
          options = (_ref = settings.options) != null ? _ref : {};
          finishOrRetry = function(err, result) {
            var callback, _ref1, _results;
            if (err != null) {
              if ((_ref1 = settings.logger) != null) {
                _ref1.error(err, "Failed to connect to `" + url + "` on startup - retrying in 5 sec");
              }
